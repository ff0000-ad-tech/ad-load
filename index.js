...
export default class Loader extends mix(Blank).with(LoaderBase) {
    ...
    if (L._total <= 0) {
      ...
      for (let l in L._queue) {
        if (!(L._queue[l] instanceof Loader)) {
          ...
          const fileAndExtension =
            extensionIndex > -1 ? fileName : fileName + "." + extension;
          _output +=
  ...
  _addSingleLoad(url, fbaOverwrite) {
    ...
    const fileType = fbaOverwrite
      ? fbaOverwrite[1]
      : L.fileType || Utils.getFileType(url);
    let loaderType;
  ...
  _addFbaSubLoads(loader) {
    ...
    for (let i = 0; i < chunkTotal; i++) {
      ...
      let type = String.fromCharCode.apply(
        String,
        new Uint8Array(loader.dataRaw, idx++, 1)
      );
      ...
      if (type === "f") {
        blobFileType =
          ...
          (fileNameSplit[1] === "ttf"
            ? "x-font-ttf"
            : `font-${fileNameSplit[1]}`);
      } else {
        blobFileType =
          "image/" + (fileNameSplit[1] == "svg" ? "svg+xml" : fileNameSplit[1]);
      }
